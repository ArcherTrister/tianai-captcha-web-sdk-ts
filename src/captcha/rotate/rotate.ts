...
import "./rotate.scss";
import {Dom, CommonCaptcha, down, initConfig, destroyEvent, showTips, closeTips} from "../common/common";
...
<div id="tianai-captcha" class="tianai-captcha-slider tianai-captcha-rotate">
    <div class="slider-tip">
        <span id="tianai-captcha-slider-move-track-font" style="font-size: ${styleConfig.i18n?.rotate_title_size || '15px'}">${styleConfig.i18n?.rotate_title || '拖动滑块完成拼图'}</span>
    </div>
    <div class="content">
...
class Rotate extends CommonCaptcha {
    ...
    init(captchaData: CaptchaData, endCallback: (data: any, captcha: Rotate) => void, loadSuccessCallback?: (captcha: Rotate) => void): Rotate {
        // 重载样式
        this.destroy();
    ...
    doMove(): void {
        ...
        if (moveX !== undefined && end) {
            this.el.find("#tianai-captcha-slider-move-btn").css("transform", "translate(" + moveX + "px, 0px)");
            this.el.find("#tianai-captcha-slider-move-img").css("transform", "rotate(" + (moveX / (end / 360)) + "deg)");
    ...
    loadCaptchaForData(that: Rotate, data: CaptchaData): void {

...
<div id="tianai-captcha" class="tianai-captcha-slider">
    <div class="slider-tip">
        <span id="tianai-captcha-slider-move-track-font" style="font-size: ${styleConfig.i18n?.slider_title_size || '15px'}">${styleConfig.i18n?.slider_title || '拖动滑块完成拼图'}</span>
    </div>
    <div class="content">
...
class Slider extends CommonCaptcha {
    ...
    init(captchaData: CaptchaData, endCallback: (data: any, captcha: Slider) => void, loadSuccessCallback?: (captcha: Slider) => void): Slider {
        // 重载样式
        this.destroy();
    ...
    loadCaptchaForData(that: Slider, data: CaptchaData): void {

...
import { StyleConfig, CaptchaData, BaseCaptcha } from "../types";
...
function createCaptchaByType(type: string, tac: TianAiCaptcha): BaseCaptcha | null {
...
class TianAiCaptcha {
    ...
    init(): TianAiCaptcha {
        ...
        this.loadStyle();
        // 绑定按钮事件
        this.config.domBindEl.find("#tianai-captcha-slider-refresh-btn").click((el: any) => {
            this.btnRefreshFun(el, this);
        });
        this.config.domBindEl.find("#tianai-captcha-slider-close-btn").click((el: any) => {
    ...
    defaultBtnRefreshFun(el: any, tac: TianAiCaptcha): void {
        tac.reloadCaptcha();
    }

    defaultBtnCloseFun(el: any, tac: TianAiCaptcha): void {
        tac.destroyWindow();
    }
    ...
    createCaptcha(): void {
        this.config.requestCaptchaData().then((data: CaptchaData) => {
            this.closeLoading();
            if (!data.code) {
                throw new Error("[TAC] 后台验证码接口数据错误!!!");
            }
            let captchaType = data.code === 200 ? data.data?.type || "DISABLED" : "DISABLED";
            const captcha = createCaptchaByType(captchaType, this);
            if (captcha == null) {
                throw new Error("[TAC] 未知的验证码类型[" + captchaType + "]");
            }
            captcha.init(data, (d: any, c: BaseCaptcha) => {
                // 验证
                ...
                const sendData: any = {
                    ...
                    tracks: currentCaptchaData.tracks,
                };
                if (c.type === 'ROTATE_DEGREE' || c.type === 'ROTATE') {
                    sendData.bgImageWidth = c.currentCaptchaData.end;
                }
                if (currentCaptchaData.data) {
                    sendData.data = currentCaptchaData.data;
                }
                // 清空
                const id = c.currentCaptchaData.currentCaptchaId;
                c.currentCaptchaData = undefined as any;
                // 调用验证接口
                this.config.validCaptcha(id!, sendData, c, this);
            });
            this.C = captcha;
            this.openCaptcha();
        });

...
## 专门为.NET版[TIANAI-CAPTCHA](https://github.com/ArcherTrister/tianai-captcha-csharp)验证码提供的前端SDK，原[TIANAI-CAPTCHA](https://gitee.com/tianai/tianai-captcha)验证码前端SDK

## TypeScript 支持

本 SDK 已移植到 TypeScript，提供完整的类型定义，支持在 TypeScript 项目中使用。

### TypeScript 示例

```typescript
import { CaptchaConfigOptions, StyleConfig } from './types';

function login() {
    // 验证码配置
    const config: CaptchaConfigOptions = {
    ...
    const style: StyleConfig = {
        logoUrl: null, // 去除logo
        // btnUrl: "https://example.com/btn.png", // 自定义按钮
        // bgUrl: "https://example.com/bg.jpg", // 自定义背景
    };
...
- `CaptchaConfigOptions`: 验证码配置选项
- `StyleConfig`: 样式配置
- `CaptchaData`: 验证码数据
- `TrackPoint`: 轨迹点
- `CaptchaConfigData`: 验证码配置数据
- `RequestParam`: 请求参数
- `RequestChain`: 请求链
- `BaseCaptcha`: 验证码基础接口
- `TianAiCaptcha`: 主验证码类接口

这些类型定义可以在 `types.ts` 文件中找到，也会在构建时生成对应的 `.d.ts` 文件。
